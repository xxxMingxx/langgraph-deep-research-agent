...
def chroma_search(
    ...
    """
    Search local knowledge base using vector retrieval.

    Args:
        query:
            User question or information needed.

    Returns:
        Relevant documents from local knowledge base.
    """
...
Source:
{doc.metadata.get("source", "unknown") if not isinstance(doc, str) else "unknown"}

Category:
{doc.metadata.get("category", "unknown") if not isinstance(doc, str) else "unknown"}
"""
        for doc in results
    ]
)
...
# RAG tool
_chroma_search_tool = tool(parse_docstring=True)(chroma_search)